#Reference: https://github.com/PRLAB21/MaxViT-UNet
...
module_path = Path("model/pytorch-image-models/timm/models/_efficientnet_blocks.py")

# Load the module
spec = importlib.util.spec_from_file_location("efficientnet_blocks", module_path)
efficientnet_blocks = importlib.util.module_from_spec(spec)
sys.modules["efficientnet_blocks"] = efficientnet_blocks
spec.loader.exec_module(efficientnet_blocks)

# Now you can import the classes you need
SqueezeExcite = efficientnet_blocks.SqueezeExcite
DepthwiseSeparableConv = efficientnet_blocks.DepthwiseSeparableConv
...
class EnhanceBranch(nn.Module):
    def __init__(self, attention_channels, decoder_channels):
        super(EnhanceBranch, self).__init__()
        self.conv1x1 = nn.Conv2d(attention_channels, decoder_channels, kernel_size=1)
        self.norm = nn.BatchNorm2d(decoder_channels)
        self.relu = nn.ReLU(inplace=True)
        
    def forward(self, attention_features, decoder_features):
        attention_features_resized = F.interpolate(attention_features, size=decoder_features.shape[2:], mode='bilinear', align_corners=True)
        enhanced_features = decoder_features + self.relu(self.norm(self.conv1x1(attention_features_resized)))
...
class MaxViTDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.enhance_branches = nn.ModuleList([
            EnhanceBranch(attention_channels=192, decoder_channels=384),
            EnhanceBranch(attention_channels=192, decoder_channels=192),
            EnhanceBranch(attention_channels=192, decoder_channels=96),
        ])
    ...
    def _forward_feature(self, inputs, attention_edge_features):
        ...
        for i in range(len(self.stages)):
            ...
            x = self.enhance_branches[i](attention_edge_features, x)
    ...
    def forward(self, inputs, attention_edge_features):
        """Forward function."""
#         TAG = self.module_name + '[forward]'
        #if self.debug: print(TAG, '[stem][output]', len(inputs))
        output = self._forward_feature(inputs, attention_edge_features)
